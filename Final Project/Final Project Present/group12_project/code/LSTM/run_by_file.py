...
# 读取指定的Excel文件
folder_path = 'T1DM'
file_name = 'processed_1006_0_20210114.xlsx'  # 替换为你想要处理的文件名
...
print(f"{file_name} has {num_rows} rows")
...
for _ in range(4):
    ...
    new_sequence = np.concatenate((last_sequence[:, 1:, :], np.concatenate((new_cgm, new_features), axis=2)), axis=1)
...
# 输出预测结果
print(f"Predicted CGM values for the last four entries for {file_name}:")
